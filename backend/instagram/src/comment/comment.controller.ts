...
export class CommentController {
  ...
  @Post()
  async createComment(@Body() commentDto: CommentModel): Promise<CommentModel> {
    if (commentDto._pid && commentDto.message) {
      if (await this.postService.exists(commentDto._pid)) {
        return this.commentService.createComment(commentDto._pid, commentDto.userId, commentDto.message);
      } else {
